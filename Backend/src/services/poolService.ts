import { ethers } from 'ethers';
  ...
  export async function getPoolReserves(pairAddress: string): Promise<{ reserveToken: number; reserveETH: number }> {
    ...
    try {
      ...
      const provider = new ethers.JsonRpcProvider(`https://eth-mainnet.g.alchemy.com/v2/${process.env.ALCHEMY_API_KEY}`, 'mainnet');
      ...
      throw new Error(`Failed to fetch pool reserves: ${error.message}`);
    }
  }